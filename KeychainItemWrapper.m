...
#else           
          // todo  [genericPasswordQuery setObject:accessGroup forKey:(id)kSecAttrAccessGroup];
#endif
        }
       ...
       // if (! SecItemCopyMatching((__bridge CFDictionaryRef)tempQuery, (CFTypeRef *)&outDictionary) == noErr)
        if (! SecItemCopyMatching((__bridge CFDictionaryRef)tempQuery, (CFTypeRef *) &attributes) == noErr)
        {
            ...
            if (accessGroup != nil)
            {
                ...
                // simulator will return -25243 (errSecNoAccessForItem).
#else           
                // todo [keychainItemData setObject:accessGroup forKey:(id)kSecAttrAccessGroup];
#endif
            }
        }
        else
...
- (void)writeToKeychain
{
    ...
    OSStatus result;
    CFDataRef attributes1;
    if (SecItemCopyMatching((__bridge CFDictionaryRef)genericPasswordQuery, (CFTypeRef *)&attributes1) == noErr)
    {
